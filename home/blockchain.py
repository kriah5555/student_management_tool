...
class Blockchain:
    def __init__(self, prev):
        print('calling constructor ===>', prev)
        self.chain = list()
        initial_block = self._create_block(
            data="genesis block", proof=1, previous_hash=prev, index=1
        )
    ...
    def _create_block(
        self, data, proof: int, previous_hash: str, index: int
    ) -> dict:
        block = {
            "index": index,
    ...
    def _proof_of_work(self, previous_proof: str, index: int, data) -> int:
        new_proof = 1
        check_proof = False
